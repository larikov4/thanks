...
(function ($, undefined) {
    ...
    function markMatch(text, term, markup, escapeMarkup, color) {
        ...
        markup.push("</span>");
        if (color && color.match(/^#([0-9a-fA-F]{6}|[0-9a-fA-F]{3})$/i)) {
        	markup.push($("<span>").addClass('color').css('background-color', color ).prop('outerHTML'));
        }
   ...
   */
    ...
    SingleSelect2 = clazz(AbstractSelect2, {
        ...
        updateSelection: function (data) {
            ...
            if (formatted !== undefined) {
            	if(data.id && data.id.match(/^#([0-9a-fA-F]{6}|[0-9a-fA-F]{3})$/i)){
            		var color = $("<span>").addClass('color').css('background-color', data.id );
    ...
    $.fn.select2.defaults = {
        ...
        formatResult: function(result, container, query, escapeMarkup) {
            var markup=[];
            markMatch(result.text, query.term, markup, escapeMarkup, result.id);
            return markup.join("");
        },
